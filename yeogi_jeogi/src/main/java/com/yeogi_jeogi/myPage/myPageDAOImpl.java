...
public class myPageDAOImpl implements myPageDAO {
	...
	@Override
	public void updateMyInfo(String pwd, String emailAdd, int userNum) throws Exception {
		int i = 1;
		boolean comma = false;
		String addSql = "";
		...
		String sql = "UPDATE USERS SET "+addSql+" WHERE USER_NO = ?";
		Connection conn = connClass.open();
		PreparedStatement pstmt = conn.prepareStatement(sql);
		if (!pwd.equals("")) {
		...
		if (!emailAdd.equals("")) {
			pstmt.setString(i, emailAdd);
			i++;
		}
		pstmt.setInt(i, userNum);
		pstmt.executeUpdate();
		pstmt.close();
		conn.close();
	}
	...
	public void updateMyImg(String imgAdd, int userNum) throws Exception {
		...
		String sql = "UPDATE USERS SET IMG_ADD = ? WHERE USER_NO = ?";
		Connection conn = connClass.open();
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, imgAdd);
