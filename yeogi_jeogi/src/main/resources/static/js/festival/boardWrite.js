...
//					$(this).addClass('dim');
	...
	var menu = $('#gnbMain').offset();
	menu.top = menu.top < 0 ? 0 : menu.top;
	...
	$(window).scroll(function() {
		...
		if ($(document).scrollTop() > menu.top) {
			...
			if(smallerThanTablet() && isSearchOpen) return;
			$('#gnbMain').removeClass('fixed');
	...
	popResize: async function(targetLayer){
		...
		*/
		...
		if(winWidth <= 1023){
			...
			// window height < popup height
			if(winHeight < popHeight){
				if(popup.parent().attr("id") != "popMemberLogin") {
					popup.css({
						'top' : winScrollTop + 15 + 'px'
					});
				}

				if(popup.parent().attr("id") == "popCoupon" ||
					popup.parent().attr("id") == "infoForm" ||
					popup.parent().attr("id") == "missionPop" ||
					popup.parent().attr("id") == "restaurant_report" ||
					popup.parent().attr("id") == "travel100Detail" ||
					popup.parent().attr("id") == "popMemberLogin"){
					popup.css({
						'top' : '50%'
					});
				}
